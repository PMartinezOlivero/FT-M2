...
// Tu código acá:
let myName = 'Pedro M.';
let textAutor = document.querySelector('#createdBy');
textAutor.innerHTML = 'Aplicación creada por ' + myName;
...
// Tu código acá:
ToDo.prototype.completeToDo = ()=> this.complete = true;
...
function buildToDos(toDos) {
  // Tu código acá:
  let myToDos = toDos.map((e,index) => buildToDo(e,index));
  return myToDos;
}
...
function displayToDos() {
  ...
  let myBuild = buildToDos(toDoItems);
  console.log(myBuild);

  for (let i = 0; i < myBuild.length; i++) { toDoContainer.appendChild(myBuild[i]); }

  //myBuild.map((e)=>toDoContainer.appendChild(e));
  //myBuild.forEach(e => toDoContainer.appendChild(e));
...
function addToDo() {
  // Tu código acá:
  let textImput = document.querySelector('#toDoInput').value;
  let newToDo = new ToDo(textImput);
...
let myButton = document.querySelector('#addButton');
